...
! make sure to call fishvel2 exactly once per day or the time units will be off: need to convert from mm/day to 1 hr timestep in growth equation (add * dtmin/3600 or leave blank)
 ...
 SUBROUTINE fishgrowth(temp,salt,rho)     ! If called from loop.F
 USE mod_param
 USE mod_fish
 USE mod_loopvars
 !USE mod_particle
...
If (stage(ntrac) .eq. f_yolk)then
   ...
   length = fish(ntrac,i_length)
   g = (0.0735+(0.0149*temp)+(-0.0013*(temp**2)))!growth in mm from B. Laurel (hatch to 10mm, unpublished data); need to convert from mm/day to 1 hr timestep (add * dtmin/3600 or blank)
   If (g .ge. 0)then
      length = length + g
   Endif
...
!POSTFLEXION: growth from 10-25 mm
 If (stage(ntrac) .eq. f_post)then
   hatchJD = fish(ntrac,i_jd)
   length = fish(ntrac,i_length)
   g = (0.0369+(0.0583*temp)+(-0.0044*(temp**2))) !growth in mm from B. Laurel (10-15 mm,unpublished data); need to convert from mm/day to 1 hr timestep (add * dtmin/3600 or blank)
   If (g .ge. 0)then
      length = length + g
   Endif
   fish(ntrac,i_length) = length
...
!TRANSFORMATION-EARLY JUVENILES: growth from 26-70 mm (45 mm to demersal age-0/end of simulation)
 If (stage(ntrac) .eq. f_ejuv)then
   hatchJD = fish(ntrac,i_jd)
   length = fish(ntrac,i_length)
   g = (0.1377+(0.0311*temp)+(0.0041*(temp**2))+(-0.0004*(temp**3))) !growth in mm from B. Laurel (40-60 mm,unpublished data); need to convert from mm/day to 1 hr timestep (add * dtmin/3600 or blank)
   If (g .ge. 0)then
      length = length + g
   Endif
   fish(ntrac,i_length) = length
